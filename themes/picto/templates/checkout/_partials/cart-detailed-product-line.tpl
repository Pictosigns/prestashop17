...
<div>{if $product.stock_quantity > 0 }<span class="btn-stock">{$product.availability_message}</span> {else} <span class="btn-stockout">{$product.available_later}</span> {/if}</div>
<div>{if $product.reference}<small class="cart_ref">{l s='ref:'}{$smarty.capture.default}{$product.reference|escape:'html':'UTF-8'}</small>{/if}</div>
